        ...
        if admin_telegram_id:
            try:
                ...
                existing_admin = self.db.execute_query(
                    'SELECT id, is_admin FROM public.users WHERE telegram_id = %s',
                    (admin_telegram_id,)
                )
                ...
                if existing_admin:
                    ...
                    if existing_admin[0][1] != 1:
                        self.db.execute_query(
                            'UPDATE public.users SET is_admin = true WHERE telegram_id = %s',
                            (admin_telegram_id,)
                        )
                    ...
                    self.db.execute_query('''
                        INSERT INTO public.users (telegram_id, name, is_admin, language, created_at)
