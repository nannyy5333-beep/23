...
class InventoryManager:
    ...
    def load_reorder_rules(self):
        ...
        for rule in rules:
            if isinstance(rule, (list, tuple)) and len(rule) >= 4:
                product_id, reorder_point, reorder_quantity, supplier_id = rule[:4]
            elif isinstance(rule, dict):
                product_id = rule.get('product_id')
                reorder_point = rule.get('reorder_point')
                reorder_quantity = rule.get('reorder_quantity')
                supplier_id = rule.get('supplier_id')
            else:
                # Неподдерживаемый формат строки – пропускаем
                continue

            if product_id is None:
                continue
            self.reorder_rules[str(product_id)] = {
    ...
    def update_stock(self, product_id, new_quantity, movement_type='manual', reason=""):
        ...
        # Получаем текущий остаток. Сравнение через id::text во избежание ошибок приведения типов UUID
        current_stock = self.db.execute_query(
            'SELECT stock FROM products WHERE id::text = %s',
            (str(product_id),)
        )
        ...
        self.db.execute_query(
            'UPDATE products SET stock = %s, updated_at = NOW() WHERE id::text = %s',
    ...
    def add_stock(self, product_id, quantity, supplier_id=None, cost_per_unit=None, reason="Поступление"):
        ...
        current_stock_rows = self.db.execute_query(
            'SELECT stock FROM products WHERE id::text = %s',
            (str(product_id),)
        )
        if not current_stock_rows:
            return False
        current_stock = current_stock_rows[0][0]
        
        new_stock = current_stock + quantity
        
        # Обновляем остаток
        self.db.execute_query(
            'UPDATE products SET stock = %s WHERE id::text = %s',
    ...
    def trigger_automatic_reorder(self, product_id):
        ...
        recent_order = self.db.execute_query('''
            ...
            WHERE product_id = %s AND created_at >= NOW() - INTERVAL '7 days'
        ''', (product_id,))
    ...
    def process_incoming_shipment(self, purchase_order_id, received_quantity, condition='good'):
        ...
        self.db.execute_query(
            'UPDATE purchase_orders SET status = %s, received_quantity = %s WHERE id = %s',
    ...
    def check_reorder_alerts(self):
        ...
        for product_id, rule in self.reorder_rules.items():
            current_stock = self.db.execute_query(
            'SELECT stock, name FROM products WHERE id::text = %s',
            (str(product_id),)
        )
    ...
    def update_stocktaking_count(self, session_id, product_id, counted_quantity):
        ...
        return self.db.execute_query('''
            ...
            SET counted_quantity = %s, counted_at = %s
    ...
    def complete_stocktaking(self, session_id):
        ...
        discrepancies = self.db.execute_query('''
            ...
            WHERE si.session_id = %s AND si.counted_quantity IS NOT NULL
    ...
    def export_inventory_csv(self, report_type):
        ...
        if report_type == 'stock_levels':
            ...
            movements = self.db.execute_query('''
                ...
                JOIN products p ON im.product_id = p.id
                LEFT JOIN suppliers s ON im.supplier_id = s.id
                WHERE DATE(im.created_at) >= (NOW() - INTERVAL '30 day')
                ORDER BY im.created_at DESC
            ''')
