...
def _safe_chat_id(msg):
    try:
        # Telebot/aiogram Message
        if hasattr(msg, "chat") and hasattr(msg.chat, "id"):
            return msg.chat.id
        # CallbackQuery style
        if hasattr(msg, "message") and hasattr(msg.message, "chat") and hasattr(msg.message.chat, "id"):
            return msg.message.chat.id
        # dict payloads
        if isinstance(msg, dict):
            if isinstance(msg.get("chat"), dict) and "id" in msg["chat"]:
                return msg["chat"]["id"]
            if isinstance(msg.get("message"), dict) and isinstance(msg["message"].get("chat"), dict):
                return msg["message"]["chat"].get("id")
            if isinstance(msg.get("from"), dict) and "id" in msg["from"]:
                return msg["from"]["id"]
        return None
    except Exception:
        return None

def _norm_uuid(val: str):
    """Return normalized UUID string or None if invalid."""
    try:
        return str(_uuid.UUID(str(val)))
    except Exception:
        return None
# ---- DB result helpers (dict/tuple safe) ----
def _as_list(rows):
    if rows is None:
        return []
...
def _first_val(rows, keys=('id',), idx=0, default=None):
    if not rows:
        return default
...
class MessageHandler:
    def __init__(self, bot, db):
        # гарантируем наличие логгера
        self.logger = getattr(self, 'logger', None) or _logger
        self.bot = bot
        self.db = db
        self.user_states = {}
...
"""
    ...
    def handle_category_selection(self, message):
        ...
        if chat_id is None:
            logging.error('Cannot determine chat_id for message: %r', message)
            return
        category_name_raw = getattr(message, "text", "") or ""
        category_name = _clean_category_text(category_name_raw)

        sql_exact = f"""
            SELECT id
            FROM categories
            WHERE name = {_sql_quote(category_name)}
            LIMIT 1
        """
        rows = _as_list(self.db.execute_query(sql_exact))

        if not rows:
            sql_like = f"""
                SELECT id
                FROM categories
                WHERE name ILIKE {_sql_quote('%' + category_name + '%')}
                ORDER BY length(name) ASC
                LIMIT 1
            """
            rows = _as_list(self.db.execute_query(sql_like))

        category_id = _first_val(rows, keys=("id", "category_id"), idx=0, default=None)
        if not category_id:
            self.logger.error(f"Категория не найдена: raw={category_name_raw!r}, parsed={category_name!r}")
            self.bot.send_message(chat_id, "Категория не найдена. Попробуйте обновить каталог.")
            return

        try:
            category_id_int = category_id if str(category_id).isdigit() else None
        except Exception:
            self.logger.exception(f"Некорректный category_id: {category_id!r}")
            self.bot.send_message(chat_id, "Ошибка категории. Попробуйте позже.")
            return
        ...
        products_sql = f"""
            ...
            LIMIT 10
        """
        products = _as_list(self.db.execute_query(products_sql, (category_id_str,)))

        if hasattr(self, "show_products_list"):
            try:
                self.show_products_list(chat_id, products)
            except Exception:
                self.logger.exception("Ошибка показа товаров.")
                self.bot.send_message(chat_id, "Не удалось показать товары. Попробуйте позже.")
        else:
            if not products:
                ...
                for p in products:
                    if isinstance(p, dict):
                        name = p.get("name") or p.get("product_name") or "Товар"
                        price = float(p.get("price") or 0)
                        stock = int(p.get("stock") or 0)
                    else:
                        name = p[1] if len(p) > 1 else "Товар"
                        price = float(p[2] if len(p) > 2 and p[2] is not None else 0)
                        stock = int(p[3] if len(p) > 3 and p[3] is not None else 0)
                    lines.append(f"• {name} — ${price:.2f} ({stock} шт.)")
    ...
    def handle_add_to_cart(self, callback_query):
        ...
        try:
            product_id = (int(data.split('_')[3]).partition(':')[2])
    ...
    def handle_show_reviews(self, callback_query):
        ...
        try:
            product_id = (int(data.split('_')[1]).partition(':')[2])
    ...
    def handle_rate_product(self, callback_query):
        ...
        try:
            parts = data.split('_')
            product_id = (int(parts[1]).partition(':')[2])
            rating = int(parts[2])
            ...
            purchased = self.db.execute_query('''
                ...
                WHERE o.user_id = %s AND oi.product_id = (%s AND o.status != 'cancelled'.partition(':')[2])
...
def handle_product_callback(self, query):
    data = query.data or ""
    prefix, _, payload = data.partition(":")
    if prefix not in {"prod_add", "prod_open"}:
        return
    pid = _norm_uuid(payload)
    if not pid:
        self.logger.error(f"Bad product id in callback: {data!r}")
        try:
            self.bot.answer_callback_query(query.id, "Не удалось определить товар.")
        except Exception:
            pass
        return
    sql = f"""
        SELECT stock, name
        FROM products
        WHERE id = {_sql_quote(pid)}::uuid
        LIMIT 1
    """
    row = _as_list(self.db.execute_query(sql))
    # дальше — твоя логика