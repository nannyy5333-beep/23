...
class ScheduledPostsManager:
    ...
    def get_target_audience(self, audience_type):
        ...
        if audience_type == 'channel':
            ...
            return self.db.execute_query('''
                ...
                WHERE u.is_admin = false AND o.created_at >= NOW() - INTERVAL '30 days'
            ''')
    ...
    def get_recipients(self, target_audience: str):
        """Return telegram_id list for a segment."""
        if not hasattr(self, 'db') or self.db is None:
            return []
        segment = (target_audience or '').strip().lower()
        if segment == 'buyers':
            rows = self.db.execute_query("""                    SELECT DISTINCT u.telegram_id
                FROM public.users u
            ...
            rows = self.db.execute_query("""                    SELECT u.telegram_id
                FROM public.users u
                WHERE u.is_admin = false
                  AND u.telegram_id IS NOT NULL
                  AND NOT EXISTS (
            ...
            rows = self.db.execute_query("""                    SELECT telegram_id
                FROM public.users
