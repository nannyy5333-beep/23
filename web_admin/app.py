...
# --- robust helper fallbacks (v5o) ---
try:
    _num_from_row
except NameError:
    def _num_from_row(row, keys, default=0):
        for k in keys:
...
try:
    ...
    def _str_from_row(row, keys, default=""):
        for k in keys:
            if isinstance(row, dict) and k in row and row[k] is not None:
                return str(row[k])
        return default
# --- end helper fallbacks ---
...
def _get_num(row, keys, default=0):
    for k in keys:
        if k in row and row[k] is not None:
            try:
                return float(row[k]) if isinstance(row[k], (int, float, str)) else default
            except Exception:
                return default
    return default

def _pair_from_row(row):
    orders = _get_num(row, ["orders", "total_orders", "cnt", "count", "order_count", "orders_count"], 0)
    revenue = _get_num(row, ["revenue", "total_revenue", "sum", "amount", "total_amount"], 0)
    return (orders, revenue)

def _triple_from_row(row):
    orders = _get_num(row, ["orders", "total_orders", "cnt", "count", "order_count", "orders_count"], 0)
    revenue = _get_num(row, ["revenue", "total_revenue", "sum", "amount", "total_amount"], 0)
    avg_check = _get_num(row, ["avg_check", "avg", "arpu", "average"], 0)
    return (orders, revenue, avg_check)
...
from supabase_db import SupabaseManager as DatabaseManager
...
@login_required
def dashboard():
    # Собираем данные для дашборда (безопасно по нулевым значениям)
    try:
        _row_today = db.execute_query('''
    ...
    try:
        _row_yesterday = db.execute_query('''
            SELECT COUNT(*) AS orders_yesterday, COALESCE(SUM(total_amount),0) AS revenue_yesterday
            FROM orders
    ...
    today_stats_tuple = (
        _num_from_row(_row_today, ['orders_today','orders','count','cnt','order_count'], 0),
        _num_from_row(_row_today, ['revenue_today','revenue','sum','total_amount'], 0),
        _num_from_row(_row_today, ['customers_today','new_customers','distinct_customers'], 0),
    )
    yesterday_stats_tuple = (
        _num_from_row(_row_yesterday, ['orders_yesterday','orders','count','cnt','order_count'], 0),
        _num_from_row(_row_yesterday, ['revenue_yesterday','revenue','sum','total_amount'], 0),
    )
    total_stats_tuple = (
        _num_from_row(_row_total, ['total_customers','customers','users','count_users'], 0),
        _num_from_row(_row_total, ['total_orders','orders','count_orders'], 0),
        _num_from_row(_row_total, ['total_revenue','revenue','sum','amount'], 0),
    )

    # Последние заказы и топ-товары
    ...
    top_products = db.execute_query('''
        SELECT p.name, SUM(oi.quantity) AS qty, COALESCE(SUM(oi.quantity * oi.price),0) AS revenue
        FROM order_items oi
    ...
    top_products_tuples = []
    for _r in top_products:
        _name = _str_from_row(_r, ['name','title','product_name'], '')
        _qty = _num_from_row(_r, ['qty','quantity','count','sold','orders','order_count'], 0)
        _rev = _num_from_row(_r, ['revenue','sum','total_revenue','amount','total_amount'], 0)
        top_products_tuples.append((_name, _qty, _rev))
    try:
        top_products_tuples.sort(key=lambda x: x[2], reverse=True)
    except Exception:
        pass

    # Безопасный максимум для прогресс-баров
    top_max = 0
    try:
        top_max = max((x[2] for x in top_products_tuples), default=0)
    except Exception:
        top_max = 0

    return render_template('dashboard.html',
        today_stats=today_stats_tuple,
        yesterday_stats=yesterday_stats_tuple,
        total_stats=total_stats_tuple,
        recent_orders=recent_orders,
        top_products=top_products_tuples,
        top_max=top_max
    )
@app.route('/orders')
@login_required
def orders():
    ...
    if search:
        query += ' AND (u.name LIKE ? OR o.id = %s)'
...
def products():
    ...
    params = []
    if q:
        where += " AND (p.name LIKE ? OR p.description LIKE %s)"
        pattern = f"%{q}%"
    ...
    # Total count
    # Robust total extraction (handles dict/list/None)
    try:
        _row_total = total if isinstance(total, dict) else (total[0] if isinstance(total, (list, tuple)) and total else {})
    except Exception:
        _row_total = {}
    total_num = _num_from_row(_row_total, ['total','count','cnt','total_count','total_products','items','rows'], 0)
    try:
        total_pages = max(1, int((total_num + per_page - 1) // per_page))
    except Exception:
        total_pages = 1
    ...
    # --- Ensure total_num by COUNT query (robust) ---
    try:
        _row_total = total if isinstance(total, dict) else (total[0] if isinstance(total, (list, tuple)) and total else None)
    except Exception:
        _row_total = None
    if not _row_total:
        try:
            params_cnt = list(params) if isinstance(params, list) else []
            cnt_rows = db.execute_query(f'''
                SELECT COUNT(*) AS total
                FROM products p
                {where}
            ''', tuple(params_cnt) if params_cnt else None)
            _row_total = cnt_rows if isinstance(cnt_rows, dict) else (cnt_rows[0] if isinstance(cnt_rows, (list, tuple)) and cnt_rows else {})
        except Exception:
            _row_total = {}
    total_num = _num_from_row(_row_total, ['total','count','cnt','total_count','total_products','rows'], 0)
    try:
        total_pages = max(1, int((total_num + per_page - 1) // per_page))
    except Exception:
        total_pages = 1
...
def add_product():
    if request.method == 'POST':
        ...
        category_id = (request.form.get('current_status') or request.form.get('status') or request.form.get('is_active') or '0')
...
def customers():
    ...
    if search:
        query += ' AND (u.name LIKE ? OR u.phone LIKE ? OR u.email LIKE %s)'
    ...
    # Threshold for 'recent' last order (last 7 days)
    try:
        recent_threshold = (datetime.utcnow() - timedelta(days=7)).strftime('%Y-%m-%d')
    except Exception:
...
def scheduled_posts():
    try:
        ...
        stats = db.execute_query('''
            ...
            WHERE ps.sent_at >= (NOW() - INTERVAL '7 day')
...
def edit_post(post_id):
    if request.method == 'POST':
        ...
        result = db.execute_query('''
            ...
            SET title = %s, content = %s, time_morning = %s, time_afternoon = %s, 
                time_evening = %s, target_audience = %s, image_url = %s, updated_at = %s
            WHERE id = %s
        ''', (
            title, content, morning_time, afternoon_time, evening_time,
    ...
    post = db.execute_query('SELECT * FROM scheduled_posts WHERE id = %s', (post_id,))
    if not post:
...
def toggle_post_status():
    ...
    current_raw = (request.form.get('current_status') or '').strip().lower()
    current_status = current_raw in ('1', 'true', 'on', 'yes')
    new_status = not current_status

    result = db.execute_query(
        'UPDATE scheduled_posts SET is_active = %s WHERE id = %s',
        (new_status, post_id)
    )
...
def delete_post():
    ...
    result = db.execute_query('DELETE FROM scheduled_posts WHERE id = %s', (post_id,))
    ...
    if result and result > 0:
        ...
        try:
            ...
            if order_details:
                ...
                user = db.execute_query('SELECT telegram_id, name FROM users WHERE id = %s', (user_id,))
...
@app.route('/toggle_product/<string:product_id>', methods=['POST'])
@login_required
def toggle_product(product_id):
    product = db.execute_query('SELECT is_active FROM products WHERE id::text = %s', (str(product_id),))
    if not product:
        flash('Товар не найден')
        return redirect(url_for('products'))

    current_status = product[0][0]
    current_raw = (request.form.get('current_status') or request.form.get('status') or request.form.get('is_active') or '0')
    current_status = str(current_raw).strip().lower() in ('1','true','on','yes')
    new_status = not current_status
    result = db.execute_query(
...
def toggle_product_status():
    ...
    current_status = (request.form.get('current_status') or request.form.get('status') or request.form.get('is_active') or '0')
    current_raw = (request.form.get('current_status') or request.form.get('status') or request.form.get('is_active') or '0')
    current_status = str(current_raw).strip().lower() in ('1','true','on','yes')
    new_status = not current_status
    result = db.execute_query(
...
def delete_product(product_id):
    product = db.execute_query('SELECT name FROM products WHERE id::text = %s', (str(product_id),))
    product_name = (product.get('name') if isinstance(product, dict) else ( (product[0].get('name') if isinstance(product[0], dict) else (str((str(product[0][0]) if isinstance(product, (list, tuple)) and product and isinstance(product[0], (list, tuple)) and len(product[0])>0 else None)) if isinstance(product[0], (list, tuple)) and len(product[0])>0 else None) ) if isinstance(product, (list, tuple)) and product else None)) or f"ID {product_id}"

    result = db.execute_query('DELETE FROM products WHERE id::text = %s', (str(product_id),))
    ...
    try:
        ...
        if target_audience == 'all':
            ...
            recipients = db.execute_query('''
                ...
                WHERE u.is_admin = 0 AND o.created_at >= NOW() - INTERVAL '30 days'
            ''')
...
def delete_category():
    ...
    try:
        ...
        flash('Некорректный ID категории — обновите страницу и попробуйте снова')
        return redirect(url_for('categories'))
    if not category_id:
...
def chart_data():
    ...
    period = int((request.args.get('period', 30) or 30))
...
def crm_quick_action():
    ...
    try:
        if action == 'send_coupon':
            ...
            db.execute_query('INSERT INTO promo_codes (code, discount_type, discount_value, is_active, created_at) VALUES (%s, %s, %s, 1, NOW())', (code, 'percent', 10))
            # try notify user
...
@app.route('/categories/toggle/<string:cid>', methods=['POST'])
@login_required
...
def financial_page():
    ...
    row_sales = sales if isinstance(sales, dict) else (sales[0] if isinstance(sales, (list, tuple)) and sales else {})
    revenue = _num_from_row(row_sales, ['revenue','total_revenue','sum','amount'], 0.0)
    orders_count = _num_from_row(row_sales, ['orders','orders_count','order_count','count'], 0)
    try:
    ...
    try:
        cost_row = db.execute_query("""
            SELECT COALESCE(SUM(oi.quantity * oi.price), 0) AS cost
            FROM order_items oi
    ...
    row_cost = cost_row if isinstance(cost_row, dict) else (cost_row[0] if isinstance(cost_row, (list, tuple)) and cost_row else {})
    cost = _num_from_row(row_cost, ['cost','sum','total_cost','amount'], 0.0)
    ...
    return render_template('financial.html', metrics=metrics, by_cat=by_cat,
                           revenue=revenue, orders_count=orders_count, aov=aov)